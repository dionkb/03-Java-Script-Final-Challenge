...
// Declare defaults for variables to be used globally. 
let passLength = 0;
let options = [];
let password = '';
let randomString =''
...
function writePassword()  {
    ...
    while (isNaN(passLength) || passLength > 128 || passLength < 8) { // This only allows input of a number from 8 to 128.
        ...
        } else if (passLength === null) { // This allows the user to cancel out of the dialog box
            return;
        }
    }    
    ...
    // and the computer repeats the random selection for "passLength" number of times.
    ...
    generatePassword();
    let password = randomString; // This stores the output of the generatPassword function to be called later
...
// Notes below were used as pseudo-code to outline a good order of debugging as well as log new issues when they arose.
// PLAN OF ATTACK:
